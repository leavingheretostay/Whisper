  ...
  return (
    <div className="page-wrapper" style={{ background: '#060614' }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '88px 20px 80px', position: 'relative', zIndex: 3 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} style={{ padding: '20px 24px', borderRadius: 20, background: 'linear-gradient(135deg, rgba(79,142,247,0.1), rgba(167,139,250,0.1))', border: '1px solid rgba(167,139,250,0.2)', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 14 }}>
          <span style={{ fontSize: 22 }}>🌙</span>
          <div><div style={{ fontSize: 11, color: 'rgba(167,139,250,0.8)', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>Today's prompt</div><p style={{ margin: 0, fontSize: 14, color: 'rgba(232,232,240,0.8)', fontStyle: 'italic' }}>"{DAILY_PROMPTS[promptIdx]}"</p></div>
        </motion.div>

        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {(['all', 'favorites'] as const).map(f => <button key={f} onClick={() => setFilter(f)} style={{ padding: '8px 20px', borderRadius: 100, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'Inter, sans-serif', transition: 'all 0.2s ease', background: filter === f ? 'rgba(79,142,247,0.2)' : 'rgba(255,255,255,0.04)', color: filter === f ? '#7eb3fa' : 'rgba(200,200,220,0.4)', border: filter === f ? '1px solid rgba(79,142,247,0.3)' : '1px solid rgba(255,255,255,0.05)' } as React.CSSProperties}>{f === 'all' ? `All (${messages.length})` : `★ Favorites (${messages.filter(m => m.is_favorited).length})`}</button>)}
        </div>

        {displayedMessages.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass" style={{ padding: '60px 24px', borderRadius: 24, textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>{EMOJIS[Math.floor(Math.random() * EMOJIS.length)]}</div>
            <h3 style={{ fontSize: 20, fontWeight: 600, marginBottom: 8 }}>{filter === 'favorites' ? 'No favorites yet' : 'Your portal is open'}</h3>
            <p style={{ color: 'rgba(200,200,220,0.5)', fontSize: 14, maxWidth: 320, margin: '0 auto 24px' }}>{filter === 'favorites' ? 'Star the messages that move you most.' : 'Share your link and let the unspoken words find you.'}</p>
